...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    ...
    find "${wavs_dir}" -name "*.wav" | sort | head -n 1000 | while read filename; do
    ...
    paste -d " " \
        <(cut -d "|" -f 1 < ${db_root}/LJSpeech-1.1/metadata.csv | head -n 1000) \
        <(cut -d "|" -f 3 < ${db_root}/LJSpeech-1.1/metadata.csv | head -n 1000) \
        > ${text}
