...
split_sets="train dev test"    # Name of the splits of the data that will be used during the whole processing.
...
acoustic_model="portuguese_mfa"    # MFA Acoustic model.
dictionary="portuguese_brazil_mfa"     # MFA Dictionary.
g2p_model="portuguese_brazil_mfa"      # MFA Grapheme-to-phoneme model.
...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
    ...
    log "Create new dictionary including OOV"
    mfa server start -q
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
    ...
    python fbscripts/utils/mfa_format.py \
        validate \
        --corpus_dir "${corpus_dir}"
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
    ...
    for dset in ${split_sets}; do
        utils/copy_data_dir.sh --validate_opts "--non-print" data/"${dset}"{,_phn}
        ...
        utils/validate_data_dir.sh --no-feats --non-print data/${dset}_phn
        #while read line ; do
        #    echo "$line 0"
        #done < data/${dset}_phn/durations > data/${dset}_phn/durations.zero
        #mv -v data/${dset}_phn/durations.zero data/${dset}_phn/durations
        fbscripts/utils/silfix.py data/${dset}_phn/{text,durations}
    done
fi
